...
function renderCarCard(car) {
  ...
  const imgSrc = car.imageUrl && String(car.imageUrl).trim() ? String(car.imageUrl).trim() : './HEROcorolla.jpg';
    ...
    <div class="car-inner">
      <div class="car-media">
        ...
        <div class="car-badges">
          ...
          <span class="pill" style="border-color: rgba(249,115,22,0.35);">₱${Number(car.pricePerDay)} / day</span>
        </div>
      </div>
      <div class="car-body">
        <div style="min-width:0;">
          <p class="car-title">${escapeHtml(car.name)}</p>
          <p class="car-meta">${escapeHtml(car.type)} • ${escapeHtml(car.brand ?? '—')} • ${Number(car.seats)} seats</p>
        </div>
...
function applyFilters(rows) {
  ...
  return rows
    .filter((c) => {
      ...
      return String(c.brand ?? '').toLowerCase() === String(brand).toLowerCase();
    })
    .filter((c) => {
      if (!q) return true;
    ...
    .slice()
    .sort((a, b) => (sort === 'low' ? a.pricePerDay - b.pricePerDay : b.pricePerDay - a.pricePerDay));
}
...
async function init() {
  showError('error', '');
  try {
    const [all, featured, brands] = await Promise.all([listCars(), listFeaturedCars(), listBrands()]);

    const featuredDeduped = dedupeCarsById(featured);
    const featuredIds = new Set(featuredDeduped.map((c) => String(c.id)));
    const allDeduped = dedupeCarsById(all).filter((c) => !featuredIds.has(String(c.id)));

    cars = allDeduped;
    featuredCars = featuredDeduped;
    brandOptions = ['All', ...(Array.isArray(brands) ? brands.map((b) => b.name) : [])];

    renderBrandOptions();
    render();
  } catch (e) {
    showError('error', e && typeof e === 'object' && 'message' in e ? String(e.message) : 'Unable to load cars');
  }
}
